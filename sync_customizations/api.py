...
from sync_customizations.utils import write_document_file, delete_document_file, get_folder, get_filters, get_doctype_field, get_file_name
...
def export_doc(doc, method=None):
    if (doc.doctype == "Custom Field" and doc.is_system_generated) or frappe.local.flags.get(IMPORT_FLAG, False):
        return
...
def on_save_customization(doc, method=None):
    frappe.db.after_commit.add(lambda: export_customizations(filter_by_doctype=doc.doc_type))
...
def export_customizations(filter_by_doctype=None):
    for doctype in DOCTYPES_TO_SYNC:
        ...
        files_to_keep = set(get_file_name(doc.name) for doc in frappe.get_all(doctype, filters=filters))
        files_to_delete = [fname for fname in os.listdir(folder) if fname not in files_to_keep]
...
def _import_customizations():
    for doctype in DOCTYPES_TO_SYNC:
        ...
        docs_to_delete = frappe.get_all(doctype, filters={"name": ("not in", docnames), **get_filters(doctype)}, pluck="name")
        if docs_to_delete:
